...
| `GENERATION_MODE`, `GENERATION_EXTRACT_*` | [Context distillation (optional)](#context-distillation-optional) |
| `GENERATION_*`, backends | [Answer generation (LLM)](#answer-generation-llm) |
| `CITATION_GRANULARITY`, `CITATION_*` | [Sentence citation attribution](#sentence-citation-attribution-optional) |
...
| `RUN_DOCUMENT` | Build document-route evidence / generation (one context per document/abstract) | `1` |
...
`GENERATION_MODE` rewrites the generation contexts before the answer prompt. Two modes:

- **`direct`** (default, or unset) — no rewriting. **Byte-identical** to the plain pipeline.
- **`claims`** — an LLM extracts atomic, query-relevant claims from the top `GENERATION_EXTRACT_TOP_N` contexts (resumable cache: `<split>_claims_cache.jsonl` next to the contexts file), then [distil_claims.py](../generation/distil_claims.py) packs deduped claims into slots by cross-encoder-ordered round-robin across source contexts.

A third value, `facets`, is accepted but **not recommended and not documented here** — it adds a clustering stage and a second LLM pass for no measured quality gain on long-form synthesis. Use `claims`.
...
| Parameter | Suggested range | Default | Notes |
|-----------|-----------------|---------|-------|
| `GENERATION_MODE` | — | **direct** | `direct` \| `claims`; ollama backend only (openai_compat fails fast) |
...
- **Models:** the ollama `think` flag is **step-dependent** for thinking models (gemma): extraction and answer generation leave the key **absent** — an explicit `false` measurably degrades both. Override per stage via `GENERATION_EXTRACT_THINK` / `GENERATION_THINK` only if you have evidence.
...
## Sentence citation attribution (optional)

Corresponds to the `CITATION_*` knobs in [workflow_config_full.env](../conf/workflow_config_full.env). Default **off**: with `CITATION_GRANULARITY` unset (or `answer`) the stage is not appended and generation output is byte-identical to the plain pipeline.

`CITATION_GRANULARITY=sentence` appends [attribute_sentences.py](../generation/attribute_sentences.py) after generation. It splits each answer into sentences, matches every sentence back to the claim text it restates, and appends `answer_sentences: [{text, doc_ids}]` with **real corpus docids**. Index remapping and cited-only reference lists are wire-format concerns and stay in each repo's adapt-out.

**Requires `GENERATION_MODE=claims`.** `direct` is refused at config time — raw passages carry no claims to match against, and a silent degraded fallback would be worse than an error.

### The method

Candidates are ranked by **RRF fusion** of a lexical order (content-word claim recall) and a semantic order (bge-m3 cosine); the top-k are cited. Fusion measured better than either signal alone — it fixes spurious word-overlap top picks without admitting any extra candidate docs — and it is the default. There is no reason to change it.

The embedding backend is imported lazily, so runs with nothing to match never load torch.
...
`CITATION_ATTRIBUTION=trivial` skips matching and gives every sentence the whole answer-level reference list — the baseline floor, useful as a control, not a production setting. Default is `auto`.
